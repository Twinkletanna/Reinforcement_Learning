...
class GridWorld:
    ...
    def softmax(self,state_num):
        m=np.max(self.q[state_num])
        new_vec = math.e**((self.q[state_num]-m)*self.temp)
    ...
    def run_sarsa(self,state,alpha,eps):
        ...
        while state!=[4,4] and i<1000:
            ...
            next_action=self.softmax(state_num) 
            # print(next_action) 
    ...
    def avgtrials(self,alpha,eps,t,n):
        ...
        for i in range(t):
            ...
            r.append(x)
            plt.plot(x)
            plt.savefig('./gwsoft/sgw'+str(i)+str(alpha)+str(eps)+'.jpg')
            plt.clf()
            np.save('./gwsoft/sgw_trial'+str(i)+str(alpha)+str(eps)+'.npy',x)
        ...
        print(vals.shape)
        plt.plot(vals)
        plt.savefig('./GridWorldsoft'+str(alpha)+str(eps)+'.jpg')
        plt.clf()
        # plt.show()
    ...
    def qlearn_ep(self,state,alpha,eps):
        ...
        while state!=[4,4] and i<1000:
            ...
            selected_action=self.softmax(state_num)
    ...
    def run_qlearn(self,alpha,eps,t,n):
        ...
        for j in range(t):
            ...
            plt.plot(one_trial)
            plt.savefig('./gwsoft/qgw_trial'+str(j)+str(alpha)+str(eps)+'.jpg')
            plt.clf()
            np.save('./gwsoft/qgw_trial'+str(j)+str(alpha)+str(eps)+'.npy',one_trial)

            all_trials.append(one_trial)

        avgd_trials=np.mean(all_trials,axis=0)
        plt.plot(avgd_trials)
        plt.savefig('./GridWorld_Qlearnsoft'+str(alpha)+str(eps)+'.jpg')
        plt.clf()
